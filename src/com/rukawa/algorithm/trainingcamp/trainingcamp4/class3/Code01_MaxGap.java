...
public class Code01_MaxGap {
    ...
    public static int maxGap(int[] nums) {
        ...
        // hasBucket[i] i号桶是否进来过数字
        boolean[] hasBucket = new boolean[len + 1];
        // maxBucket[i] i号桶收集的所有数字的最大值
        int[] maxBucket = new int[len + 1];
        // minBucket[i] i号桶收集的所有数字的最小值
        int[] minBucket = new int[len + 1];
        int bid = 0;
        for (int i = 0; i < len; i++) {
            bid = bucket(nums[i], len, min, max);
            minBucket[bid] = hasBucket[bid] ? Math.min(minBucket[bid], nums[i]) : nums[i];
            maxBucket[bid] = hasBucket[bid] ? Math.max(maxBucket[bid], nums[i]) : nums[i];
            hasBucket[bid] = true;
        }
        int res = 0;
