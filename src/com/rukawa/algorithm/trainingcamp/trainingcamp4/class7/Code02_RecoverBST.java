...
public class Code02_RecoverBST {
     ...
     * 值都不一样，给定二叉树的头节点 head，返回一个长度为2的 二叉树节点类型
     * 的数组errs，errs[0]表示一个错误节点， errs[1]表示另一个错误节点。
     *
     * 进阶: 如果在原问题中得到了这两个错误节点，我们当然可以通过交换两个节点
     * 的节点值的方 式让整棵二叉树重新成为搜索二叉树。但现在要求你不能这么做，
     * 而是在结构上完全交换两个节点的位置，请实现调整的函数
     */
    ...
    public static Node[] getTwoErrNodes(Node head) {
        /**
         * 思路：
         *  1、中序遍历，二叉搜索数是升序，找到其中的降序过程
         *  2、找到第一次降序的头一个节点和第二次降序的第二个节点
         *  3、不可能一定出现两次降序，也有可能只出现一次降序，左孩子和它的父亲节点为两个错误节点
         */
