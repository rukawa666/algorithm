...
public class Code03_VisibleMountains {
     ...
     * {4,5,3,1,2}或{1,2,4,5,3}都代表同样结构的环形山。
     * 山峰A和山峰B能够相互看见的条件为：
     * 1、如果A和B是同一座山，认为不能互相看见。
     * 2、如果A和B是不同的山，并且在环中相邻，认为可以互相看见
     * 3、如果A和B是不同的山，并在在环中不相邻，假设两座山高度的最小值为min。
     *   a、如果A通过顺时针方向到B的途中没有高度比min大的山峰，认为A和B互相看见
     *   b、如果A通过逆时针方法到B的途中没有高度比min大的山峰，认为A和B互相看见
     *   c、两个方向只要有一个能看见，就算A和B可以互相看见
     * 给定一个不含有负数且没有重复值的数组arr,请返回有多少对山峰能够互相看见。
     * 进阶：给定一个不含有负数但可能含有重复值的数组arr，返回有多少对山峰能互相看见
     */
    ...
    // 不含有负数且没有重复值
    // O(1)
    public static int getVisibleNum1(int[] arr) {
        /**
         * 思路：可以打表
         * 1、找到数组中的最大值和次大值
         * 2、然后剩下的N-2的数都能找到2对
    ...
    public static int getVisibleNum(int[] arr) {
        ...
        while (index != maxIndex) {
            // 当前数要进入循环栈，判断会不会破坏第一维的数字从顶到底依次变大
            // 如果破坏了，就依次弹出栈顶记录，并计算山峰对数量
            while (stack.peek().value < arr[index]) {
                int k = stack.pop().times;
                // 弹出记录为(X,Y)，如果K==1，产生两对，如果K>1，产生2 * K + C(2,K)对
    ...
    // 如果k==1，返回0，如果k>1返回C(2,k)
    public static int getInternalSum(int k) {
        return k == 1 ? 0 : (k * (k - 1) / 2);
    }

    // 环境数组中当前位置为i，数组长度为size，返回i的下一个位置
